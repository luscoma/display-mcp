...
Ten MCP tools: `set_display` publishes a document, `copy_display` republishes
one name's document under another without resending the body, `preview`
returns a rendered PNG so I can see what the wall will look like before
publishing, `validate` checks a draft without publishing, `get_display`
reads back what's live, `status` reports whether the panel has collected it,
`clear_display` takes a display down, `describe` returns the op vocabulary as
JSON, `guide` returns the prose composing guide, and `swatches` renders
every named colour
as a labelled chip — a sheet that is itself a document; asking it for
`include_document` returns that document too, to `set_display` and put
every one of them on the wall. `compose_display` is a prompt that
