...
| `copy_display` | `source`, `name` | `{name, hash, etag, ops, bytes, warnings, recent_fetch_at, recent_fetch_ago}`, same shape as `set_display` — republishes `source`'s document under `name` (`store.publish(store.get(source).doc, name=name)`): same hash, fresh `generated`, `first_fetch_at` on `name` resets like any publish (and stays `null` while the panel 304s a copy whose hash it already showed). `source == name` is a plain republish. Unknown `source` is a tool error. |
