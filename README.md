...
arguments, for a client that cannot reach resources or prompts. `swatches`
renders every ink and built-in mix as a labelled, flat-coloured chip — the
sheet is itself a valid document; pass `include_document=true` to get it as
a third block and `set_display` it, putting every named colour on the wall.
